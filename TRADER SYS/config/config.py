...
every other module already defines:
    ...
    config/trading.py
        ...
        download limit, prediction horizon, Kronos inference settings,
...
from prediction_agent.kronos_wrapper import KronosWrapperConfig, timeframe_to_timedelta
...
def _validate_trading_parameters(params: TradingParameters, errors: _ErrorCollector) -> None:
    ...
    if params.prediction_horizon_minutes <= 0:
        errors.add(
            f"config/trading.py: prediction_horizon_minutes={params.prediction_horizon_minutes} "
            "must be positive"
        )
    # The actual horizon -> pred_len division (and the "at least one
    # bar" check) happens once, at construction time in load_config()
    # — see the Kronos section there — rather than being duplicated
    # here, since it needs trading_timeframe successfully parsed first
    # and load_config() already has a single place that does that.
...
def load_config() -> DeploymentConfig:
    ...
    # pred_len is a Kronos implementation detail (a bar count), derived
    # here from the trading-domain configuration (prediction_horizon_minutes
    # and trading_timeframe) rather than being independently set —
    # see config/trading.py's own docstring for the full rationale.
    try:
        timeframe_minutes = timeframe_to_timedelta(params.trading_timeframe).total_seconds() / 60
        pred_len = int(params.prediction_horizon_minutes // timeframe_minutes)
    except Exception as exc:
        errors.add(
            f"config/trading.py: could not derive pred_len from prediction_horizon_minutes="
            f"{params.prediction_horizon_minutes} / trading_timeframe={params.trading_timeframe!r}: {exc}"
        )
        pred_len = 1  # safe placeholder so construction can still complete; load_config() raises below regardless
    else:
        if pred_len < 1:
            errors.add(
                f"config/trading.py: prediction_horizon_minutes={params.prediction_horizon_minutes} "
                f"is shorter than one trading_timeframe bar ({params.trading_timeframe}) — "
                "derived pred_len would be 0, which Kronos cannot forecast with"
            )
            pred_len = 1
        elif params.prediction_horizon_minutes % timeframe_minutes != 0:
            effective_horizon = pred_len * timeframe_minutes
            logger.warning(
                "prediction_horizon_minutes=%s is not an exact multiple of trading_timeframe=%s "
                "(%s minutes/bar) — using pred_len=%d, an effective horizon of %s minutes "
                "(rounded down) rather than %s",
                params.prediction_horizon_minutes, params.trading_timeframe, timeframe_minutes,
                pred_len, effective_horizon, params.prediction_horizon_minutes,
            )
    ...
    kronos_wrapper_config = KronosWrapperConfig(
        ...
        # present), not a strategy choice — stays in .env. Left unset
        # (None) here means "auto-detect," which KronosPredictor's own
        # __init__ already implements as CUDA -> MPS -> CPU, in that
        # order (see kronos/kronos.py) — this project does not need to
        # (and does not) hardcode CUDA-only behavior anywhere; leaving
        # KRONOS_DEVICE unset in .env is sufficient to "use CUDA
        # whenever available, fall back to CPU automatically."
    ...
    #
    # validity_window is likewise fully determined, not an independent
    # choice: a Prediction Artifact's forecast covers exactly
    # pred_len * timeframe into the future (the same derived pred_len
    # above), so that is also how long it remains meaningfully valid.
    # There is now exactly one configured value —
    # prediction_horizon_minutes — that both pred_len and validity_window
    # trace back to; they can never independently drift apart again.
    validity_window = pred_len * timeframe_to_timedelta(params.trading_timeframe)
    artifact_builder_config = ArtifactBuilderConfig(

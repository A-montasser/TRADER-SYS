...
def _format_utc_and_local(dt: datetime) -> str:
    """
    Task 7 (logging UX): UTC remains authoritative everywhere
    internally — every comparison and every persisted value in this
    project stays naive UTC, unchanged. This only affects what a
    human reads in a log line: showing both UTC and the operator's
    local time side by side removes the confusion of a naive-UTC
    value sitting next to a locally-timestamped log line prefix with
    no label (see the Task 2 finding from the prior production
    review). Assumes `dt` is naive UTC, this project's one consistent
    convention.
    """
...
class ArtifactBuilderConfig:
    ...
    pred_len/timeframe are not a separate concept — the caller passes
    through the same values already used to construct
    KronosWrapperConfig (prediction_agent/kronos_wrapper.py), so the
    forecast horizon has a single source of truth rather than being
...
def build_artifact(
    ...
    logger.info(
        ...
        artifact.artifact_id, len(records), len(forecast_results),
        _format_utc_and_local(artifact.valid_until),
    )
    return artifact
...
def _flatten_records(artifact: PredictionArtifact) -> pd.DataFrame:
    ...
    ranking_position/ranking_score/artifact_id are denormalized
    (repeated) across all bar rows for a given symbol.

    artifact_id is included so trading/artifact_loader.py can detect a
    parquet/meta.json pairing mismatch (e.g. a crash between the two
    os.replace() commits in save_artifact() leaving a new parquet paired
    with a stale meta.json) by comparing this column against
    meta.json's artifact_id, independent of the expiry check.
    """
    rows = []

...
def _fetch_metrics(
    exchange: Exchange, symbols: List[str], filter_config: FilterConfig
) -> Optional[Dict[str, MarketMetrics]]:
    """
    ...
    Skipped entirely when neither liquidity/spread threshold is
    configured: filters.py's filter_by_market_metrics() already
    short-circuits to a no-op in that exact case (see its own
    docstring), so the only remaining consumer of `metrics` would be
    ranking.py's exact-momentum-tie tie-break — not worth ~14 batched
    Binance requests per cycle for a value that changes the outcome
    only on a tie. This does not change ranking's primary sort
    behavior (momentum_factor), only what breaks a tie in the rare
    case one occurs.
    """
    if not symbols:
        return None
    if filter_config.min_quote_volume_24h is None and filter_config.max_spread_pct is None:
        return None
    try:
...
def run_prediction_cycle(
    ...
    metrics = _fetch_metrics(exchange, [c.symbol for c in candidates], filter_config)

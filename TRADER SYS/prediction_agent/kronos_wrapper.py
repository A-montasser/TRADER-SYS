...
def timeframe_to_timedelta(timeframe: str) -> pd.Timedelta:
    ...
    Public (not module-private) so config/config.py can reuse it when
    deriving pred_len and artifact validity from a single configured
    prediction horizon, rather than duplicating this parsing logic.
    """
...
class KronosWrapper:
    ...
    def __init__(self, config: KronosWrapperConfig):
        ...
        self._config = config
        self._interval = timeframe_to_timedelta(config.timeframe)
        logger.info(

...
Responsibility (frozen):
    Transform raw ForecastResult objects into objective, descriptive
    forecast analytics (return, range, volatility, trend, drawdown/
    upside estimates). Purely mathematical — no trading decisions.
...
from models.artifact import ForecastAnalytics, ForecastResult
...
def compute_analytics(forecast_result: ForecastResult) -> ForecastAnalytics:
    ...
    Raises:
        AnalyticsError: if the forecast has fewer than 2 bars (analytics
        such as slope/return are undefined for a single point).
    """
...
def compute_analytics_batch(
    ...
    failure (e.g. degenerate forecast) is logged and skipped — does not
    abort the batch, consistent with downloader.py/validator.py's

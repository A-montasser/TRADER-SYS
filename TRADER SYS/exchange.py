...
class Exchange:
    ...
    # Binance's documented limit for the `symbols` array parameter on
    # GET /api/v3/ticker/24hr is 100 symbols per request — exceeding it
    # is rejected outright, which is exactly what was happening here
    # (1366 scanned symbols requested in one call). This is a Binance
    # API business rule, not a URL-length/infra issue, so the fix is a
    # batching loop, not a payload-size workaround.
    _TICKER_BATCH_SIZE = 100

    def _truncate_for_log(self, text: str, limit: int = 300) -> str:
        """
        Caps how much of a raw ccxt exception message (which can embed
        a full request URL — e.g. a long `symbols=[...]` query string)
        reaches the log. Keeps the actionable prefix (error type,
        HTTP status if present) and drops the rest rather than
        dumping several hundred/thousand characters of URL per line.
        """
        text = str(text)
        if len(text) <= limit:
            return text
    ...
    def fetch_market_metrics(self, symbols: List[str]) -> Dict[str, MarketMetrics]:
        ...
        Batches `symbols` into groups of at most _TICKER_BATCH_SIZE
        (Binance's own documented cap for this endpoint's `symbols`
        parameter) and merges the results. Each batch is independently
        best-effort: a single failed batch is logged and skipped rather
        than failing the entire call, so one bad/delisted symbol in a
        1300-symbol universe can't discard metrics for the other 1299.
        Only raises ExchangeError if every batch fails.
        """
        result: Dict[str, MarketMetrics] = {}
        batches_failed = 0
        batches_total = 0

        for start in range(0, len(symbols), self._TICKER_BATCH_SIZE):
            batch = symbols[start:start + self._TICKER_BATCH_SIZE]
            batches_total += 1
            try:
                raw_tickers = self._exchange.fetch_tickers(batch)
            except Exception as exc:
                batches_failed += 1
                logger.warning(
                    "Failed to fetch tickers for batch %d/%d (%d symbols): %s",
                    batches_total, -(-len(symbols) // self._TICKER_BATCH_SIZE), len(batch),
                    self._truncate_for_log(exc),
                )
                continue
        ...
        if batches_total > 0 and batches_failed == batches_total:
            raise ExchangeError(
                f"Failed to fetch tickers: all {batches_total} batch(es) failed"
            )

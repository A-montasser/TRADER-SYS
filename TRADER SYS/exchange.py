...
class Exchange:
    ...
    def create_market_buy_order(self, symbol: str, amount: float) -> OrderResult:
        try:
            order = self._exchange.create_market_buy_order(
                symbol, amount, params={"type": "spot", "recvWindow": 10000}
            )
        except Exception as exc:
            raise ExchangeError(f"Buy order failed for {symbol}: {exc}") from exc
        return self._translate_order(order, symbol, "buy")

    def create_market_sell_order(self, symbol: str, amount: float) -> OrderResult:
        try:

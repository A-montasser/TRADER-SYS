...
class PredictionArtifact:
    ...
    pred_len / timeframe expose the Prediction Agent's forecast horizon
    and bar interval (e.g. pred_len=24, timeframe="5m") so the Trading
    Agent can reason about forecast timing directly from the artifact
    instead of relying on external/duplicated configuration. Like
    valid_from/valid_until, these are run-level facts — predict_batch()
    enforces an identical horizon and timeframe for every symbol in one
    Prediction Cycle, so they belong to the artifact, not per-record.
    """
    artifact_id: UUID

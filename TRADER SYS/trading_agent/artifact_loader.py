...
def load_artifact(parquet_path: Path, meta_path: Path, enforce_expiry: bool = True) -> PredictionArtifact:
    """
    ...
    Validates (in order):
        ...
        - artifact expiry (valid_until vs. now) — skipped if
          enforce_expiry=False
        - ranking_position / ranking_score consistency within each symbol

    Args:
        parquet_path / meta_path: the artifact's files.
        enforce_expiry: True (default) preserves the original behavior
            — reject an artifact whose valid_until has passed. Callers
            loading a Position Forecast (the exact artifact that
            justified a currently open Position's entry — see
            trading_bot.py) pass False: that artifact must remain
            loadable for the position's entire lifetime regardless of
            its original validity window, since it is still the
            authoritative timeline reference for that position's
            HOLD/EXIT evaluation. The artifact_id consistency and
            schema checks above still apply either way — this only
            affects the freshness check, not integrity.
    ...
    if enforce_expiry and valid_until <= now:
        raise ArtifactLoaderError(
            f"Artifact {artifact_id} expired: valid_until={_format_utc_and_local(valid_until)}, "
            f"now={_format_utc_and_local(now)}"
        )
    ...
    logger.info(
        ...
        artifact.artifact_id, len(records), _format_utc_and_local(artifact.valid_until),
    )
    return artifact
...
def build_trade_record(
    ...
    new_balance/exit_reason are caller-supplied rather than computed
    here: this module does not allocate capital or make decisions, so
    it cannot derive either value itself.
    ...
    Args:
        ...
        new_balance: Trading Agent budget after this trade, as tracked
            by the caller.
    ...
    logger.info(
        "Trade record built: %s pnl=%.8f (%.4f%%) duration=%.2fmin reason=%s",
        record.symbol, record.pnl, record.pnl_pct, record.duration_min, record.exit_reason,
    )
    return record

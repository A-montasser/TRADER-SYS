...
def run_trading_cycle(
    ...
    Returns:
        TradingCycleResult
    """
    artifact = load_artifact(parquet_path, meta_path)
    ...
    if decision.action == DecisionAction.ENTRY and decision.opportunity is not None:
        ...
        if allocated > 0:
            try:
                buy_result = submit_buy_order(exchange, symbol, allocated)
        ...
        try:
            sell_result = submit_sell_order(exchange, open_position.symbol, open_position.amount)
        except ExecutionError as exc:
            ...
            new_budget = trade_record.new_balance
            new_open_position = None

            # Meta-learning only has new information to analyze once a
            # trade has actually closed — running it on every WAIT/HOLD
            # cycle would recompute identical results for no reason.

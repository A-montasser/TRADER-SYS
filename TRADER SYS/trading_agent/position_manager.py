...
Implementation constraints (flagged, not resolved here):
    - stop_loss/take_profit: Position requires these as entry-time
      facts. Ownership of computing them has been assigned to
      risk_manager.py (forecast-path-derived, no formula decided yet)
      — create_position() still accepts them as given parameters
      rather than computing them, since this module manages lifecycle
      only, not risk.
